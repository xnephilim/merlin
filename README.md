...
- **[API](https://api.ethereum.xnephilim.com/docs/)** - provides a base set of functionality via REST and WebSocket that can be extended with coin specific logic
...
- All paths are relative to the root hightable project directory (ex. `hightable/[go|node]/{path}`)
...
- Each language subdirectory has setup requirements before running a coinstack locally
  - [Go](go/README.md#initialsetup) - `hightable/go`
  - [Node](node/README.md) - `hightable/node`
...
- Mac OS: once you start a coinstack you should be able to access hightable in the browser without further config, but for CLI access to work you need to modify `/etc/hosts` and add a valid DNS entry:
