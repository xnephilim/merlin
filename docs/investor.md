...
You can look at [@xblackfury/investor-yearn](https://github.com/xnephilim/lib/tree/main/packages/investor-yearn) as an example implementation.
...
`assetId` must be of type `AssetId` from `@xblackfury/caip` and is a CAIP-19 representation of the asset or token that can be deposited into this `Opportunity`.

For example, `AssetId` for Ethereum is `eip155:1/slip44:60`

The `@xblackfury/asset-service` package has a full list of `AssetIds` that we currently support. You can submit a PR to that package to add new assets.
