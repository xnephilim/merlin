...
import (
	...
	"github.com/xnephilim/hightable/coinstacks/thorchain"
	"github.com/xnephilim/hightable/pkg/api"
...
func (h *Handler) ParseFee(tx signing.Tx, txid string, denom string) cosmos.Value {
	...
	// TODO: query and cache value returned from the node at https://daemon.thorchain.xnephilim.com/lcd/thorchain/constants
...
func (h *Handler) getTxFromEndBlockEvents(eventCache map[string]interface{}, blockHeader types.Header, endBlockEvents []abci.Event, eventIndex int) (*ResultTx, error) {
	// attempt to find matching fee event for txid or use default fee as defined by https://daemon.thorchain.xnephilim.com/lcd/thorchain/constants
