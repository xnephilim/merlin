...
import { fromChainId, toAssetId } from '@xblackfury/caip'
...
export class Parser implements SubParser<Tx> {
  ...
  async parse(tx: Tx): Promise<TxSpecific | undefined> {
    ...
    const isMerlinStakingRewards = UNI_V2_MERLIN_STAKING_REWARDS_CONTRACTS.some(contract =>
      txInteractsWithContract(tx, contract),
    )

    if (isMerlinStakingRewards) return await Promise.resolve(this.parseStakingRewards(tx))
  }
