import { ChainId, toAssetId } from '@xblackfury/caip'
...
import xnephIlimRouter from './abi/xnephIlimRouter'
...
export class Parser implements SubParser<Tx> {
  ...
  readonly xnephIlimInterface = new ethers.utils.Interface(xnephIlimRouter)
  ...
  readonly supportedShapeShiftFunctions = {
    depositSigHash: this.xnephIlimInterface.getSighash('deposit(address,address,uint256,uint256)'),
  }
  ...
  getAbiInterface(txSigHash: string | undefined): ethers.utils.Interface | undefined {
    ...
    if (Object.values(this.supportedShapeShiftFunctions).some((abi) => abi === txSigHash))
      return this.xnephIlimInterface
    return undefined
  }
}
