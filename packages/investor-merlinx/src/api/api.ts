...
export class MerlinxApi {
  ...
  public web3: Web3
  private merlinxStakingContracts: Contract[]
  ...
  constructor({
    ...
    this.merlinxStakingContracts = merlinxAddresses.map(
      (addresses) => new this.web3.eth.Contract(merlinxStakingAbi, addresses.staking),
    )
  ...
  async getMerlinxOpportunityByStakingAddress(stakingAddress: string) {
    ...
    const addresses = this.merlinxAddresses.find(async (item) => {
  ...
  async tvl(input: TokenAddressInput): Promise<BigNumber> {
    ...
    this.verifyAddresses([tokenContractAddress])
    const contract = new this.web3.eth.Contract(merlinxAbi, tokenContractAddress)
  ...
  async getRebaseHistory(input: BalanceInput) {
    ...
    const merlinxContract = new this.web3.eth.Contract(merlinxAbi, tokenContractAddress)
    const fromBlock = 14381454 // genesis rebase

    const rebaseEvents = await (async () => {
      try {
        const events = (
          await merlinxContract.getPastEvents('LogRebase', {
            fromBlock,
            toBlock: 'latest',
          })
    ...
    const transferEvents = await (async () => {
      try {
        const events = await merlinxContract.getPastEvents('Transfer', {
          fromBlock,
          toBlock: 'latest',
        })
        return events
      ...
      events.map(async (event) => {
        const { preRebaseBalance, postRebaseBalance } = await (async () => {
          try {
            ...
            const postRebaseBalanceResult = await merlinxContract.methods
              .balanceOf(userAddress)
              .call(null, event.blockNumber)
            const unadjustedPreRebaseBalance = await merlinxContract.methods
              .balanceOf(userAddress)
