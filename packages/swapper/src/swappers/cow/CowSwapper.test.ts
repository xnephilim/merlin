...
import { BTC, ETH, MERLIN, WBTC, WETH } from '../utils/test-data/assets'
...
const ASSET_IDS = [ETH.assetId, WBTC.assetId, WETH.assetId, BTC.assetId, MERLIN.assetId]
...
describe('CowSwapper', () => {
  ...
  describe('getUsdRate', () => {
    it('calls getUsdRate on swapper.getUsdRate', async () => {
      await swapper.getUsdRate(MERLIN)
      expect(getUsdRate).toHaveBeenCalledWith(COW_SWAPPER_DEPS, MERLIN)
    })
  })
  ...
  describe('filterAssetIdsBySellable', () => {
    ...
    it('returns array filtered out of unsupported tokens', () => {
      const assetIds = [MERLIN.assetId, 'eip155:1/erc20:0xdc49108ce5c57bc3408c3a5e95f3d864ec386ed3']
      expect(swapper.filterAssetIdsBySellable(assetIds)).toEqual([MERLIN.assetId])
    })
  })
  ...
  describe('filterBuyAssetsBySellAssetId', () => {
    ...
    it('returns array filtered out of non erc20 tokens when called with a sellable sellAssetId', () => {
      ...
      ).toEqual([ETH.assetId, WBTC.assetId, MERLIN.assetId])
      expect(
        swapper.filterBuyAssetsBySellAssetId({
          assetIds: ASSET_IDS,
          sellAssetId: WBTC.assetId,
        }),
      ).toEqual([ETH.assetId, WETH.assetId, MERLIN.assetId])
      expect(
        swapper.filterBuyAssetsBySellAssetId({
          assetIds: ASSET_IDS,
          sellAssetId: MERLIN.assetId,
        }),
    ...
    it('returns array filtered out of unsupported tokens when called with a sellable sellAssetId', () => {
      ...
      ).toEqual([MERLIN.assetId])
      expect(swapper.filterBuyAssetsBySellAssetId({ assetIds, sellAssetId: MERLIN.assetId })).toEqual(
        [],
      )
    })
  })
