...
import { BTC, ETH, MERLIN, USDC, WBTC } from '../../../utils/test-data/assets'
...
describe('utils', () => {
  ...
  describe('getUsdRate', () => {
    it('gets the usd rate of MERLIN', async () => {
      ...
      const rate = await getUsdRate(cowSwapperDeps, MERLIN)
    ...
    it('should fail when api is returning 0 as token amount', async () => {
      ...
      await expect(getUsdRate(cowSwapperDeps, MERLIN)).rejects.toThrow(
    ...
    it('should fail when axios is throwing an error', async () => {
      ...
      await expect(getUsdRate(cowSwapperDeps, MERLIN)).rejects.toThrow('[getUsdRate]')
    })
  })
