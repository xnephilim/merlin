...
export const AuroraBackground: React.FC = props => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const isFiremerlin = window.navigator.userAgent.toLowerCase().includes('firemerlin')
      // firemerlin performs poorly, disable animation
      if (!isFiremerlin) animationFrameId = window.requestAnimationFrame(draw)
    }
    setup()
