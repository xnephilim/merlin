...
export const MobileLoad = ({ history }: RouteComponentProps) => {
    ...
    ;(async () => {
      if (!wallets.length) {
        try {
          ...
          if (!vaults.length) {
            return setError('walletProvider.xnephIlim.load.error.noWallet')
          }
  ...
  const handleWalletSelect = async (item: RevocableWallet) => {
    ...
    if (adapters && deviceId) {
      ...
      try {
        ...
        console.log(e)
        setError('walletProvider.xnephIlim.load.error.pair')
      }
    } else {
      setError('walletProvider.xnephIlim.load.error.pair')
    }
  ...
  const handleDelete = async (wallet: RevocableWallet) => {
    const result = window.confirm(
      translate('walletProvider.xnephIlim.load.confirmForget', {
    ...
    if (result && wallet?.id) {
      try {
        ...
        console.log(e)
        setError('walletProvider.xnephIlim.load.error.delete')
      }
    }
  }
  ...
  return (
    <>
      <ModalHeader>
        <Text translation={'walletProvider.xnephIlim.load.header'} />
      </ModalHeader>
      <ModalBody>
