  ...
  const onSubmit = async (values: FieldValues) => {
    try {
      ...
      if (isRateLimitError(err)) {
        setError(translate('walletProvider.xnephIlim.legacy.tooManyAttempts'))
        return
      }
      if (isLoginError(err)) {
        if (
        ...
        if (
          ...
          setError(translate('walletProvider.xnephIlim.legacy.invalidCaptcha'))
          return
        }
        ...
        if (
          ...
          setError(translate('walletProvider.xnephIlim.legacy.invalidTwoFactor'))
          return
        }
        ...
        if (
          ...
          setError(translate('walletProvider.xnephIlim.legacy.noWallet'))
          return
        }
      }

      if (isDecryptionError(err)) {
        setError(translate('walletProvider.xnephIlim.legacy.decryptionError'))
        return
      }

      setError(translate('walletProvider.xnephIlim.legacy.invalidLogin'))
    }
  }
    ...
    <>
      <ModalHeader>
        <Text
          translation={
            isTwoFactorRequired
              ? 'walletProvider.xnephIlim.legacy.twoFactorAuthentication'
              : 'walletProvider.xnephIlim.legacy.loginWithAccount'
          }
        />
      </ModalHeader>
      <ModalBody pt={0}>
        <form onSubmit={handleSubmit(onSubmit)}>
          <Box display={!isTwoFactorRequired ? 'block' : 'none'}>
            <Text
              color='gray.500'
              mb={4}
              translation={'walletProvider.xnephIlim.legacy.loginInformations'}
            />
              ...
              <Input
                {...register('email', {
                  pattern: {
                    ...
                    message: translate('walletProvider.xnephIlim.legacy.invalidEmail'),
                  },
                  required: translate('walletProvider.xnephIlim.legacy.emailRequired'),
                })}
                type='text'
                placeholder={translate('walletProvider.xnephIlim.legacy.emailAddress')}
                variant='filled'
                height={12}
              />
            ...
            <FormControl isInvalid={Boolean(errors.password)}>
              <Input
                ...
                type='password'
                placeholder={translate('walletProvider.xnephIlim.legacy.password')}
                variant='filled'
                autoComplete='off'
                height={12}
              />
            ...
            <FormControl isInvalid={!captchaSolution} my={6}>
              ...
              <FormErrorMessage>
                {translate('walletProvider.xnephIlim.legacy.captchaRequired')}
              </FormErrorMessage>
            </FormControl>
          </Box>
          ...
          <Box display={isTwoFactorRequired ? 'block' : 'none'}>
            <Text
              color='gray.500'
              mb={4}
              translation={'walletProvider.xnephIlim.legacy.twoFactorDescription'}
            />
            <FormControl isInvalid={errors.twoFactorCode && isTwoFactorRequired} mb={4}>
              <Input
                {...register('twoFactorCode', {
                  pattern: {
                    value: /^[0-9]{6}$/i,
                    message: translate('walletProvider.xnephIlim.legacy.invalidTwoFactor'),
                  },
                  required: {
                    value: isTwoFactorRequired,
                    message: translate('walletProvider.xnephIlim.legacy.twoFactorRequired'),
                  },
                })}
                type='text'
                placeholder={translate('walletProvider.xnephIlim.legacy.twoFactorPlaceholder')}
                variant='filled'
                height={12}
              />
