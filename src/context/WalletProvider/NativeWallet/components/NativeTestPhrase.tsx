...
export const NativeTestPhrase = ({ history, location }: NativeSetupProps) => {
  ...
  const shuffleMnemonic = useCallback(async () => {
    ...
    try {
      ...
      setError('walletProvider.xnephIlim.create.error')
    }
  }, [setTestState, shuffledNumbers, vault, revoker, testCount])

  useEffect(() => {
    shuffleMnemonic().catch(() => setError('walletProvider.xnephIlim.create.error'))
  }, [shuffleMnemonic])
  ...
  return !testState ? null : (
    <>
      <ModalHeader>
        <Text translation={'walletProvider.xnephIlim.testPhrase.header'} />
      </ModalHeader>
      <ModalBody>
        <RawText>
          <Text
            as='span'
            color='gray.500'
            translation={'walletProvider.xnephIlim.testPhrase.body'}
          />{' '}
          <Tag colorScheme='green'>
            {translate(
              `walletProvider.xnephIlim.testPhrase.${testState.targetWordIndex + 1}${ordinalSuffix(
                testState.targetWordIndex + 1,
              )}`,
            )}
            <Text as='span' ml={1} translation={'walletProvider.xnephIlim.testPhrase.body2'} />
          </Tag>{' '}
          <Text
            as='span'
            color='gray.500'
            translation={'walletProvider.xnephIlim.testPhrase.body3'}
          />
        </RawText>
        <Wrap mt={12} mb={6}>
          {testState &&
        ...
        {isLegacyWallet && (
          <Box>
            ...
            <Checkbox mb={4} spacing={4} onChange={onCheck} isChecked={hasAlreadySaved}>
              <Text
                fontSize='sm'
                translation={'walletProvider.xnephIlim.legacy.alreadySavedConfirm'}
              />
            </Checkbox>
            <Button
              colorScheme='blue'
