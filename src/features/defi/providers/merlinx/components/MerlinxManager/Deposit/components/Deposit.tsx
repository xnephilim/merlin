...
export const Deposit: React.FC<DepositProps> = ({
  ...
  const opportunity = useMemo(() => state?.merlinxOpportunity, [state])
  ...
  const handleContinue = useCallback(
    async (formValues: DepositValues) => {
      if (!(state && accountAddress && dispatch && merlinxApi)) return

      const getApproveGasEstimate = async () => {
        if (!accountAddress || !assetReference || !merlinxApi) return
        try {
          const feeDataEstimate = await merlinxApi.estimateApproveFees({
      ...
      const getDepositGasEstimateCryptoBaseUnit = async (deposit: DepositValues) => {
        if (!accountAddress || !assetReference || !merlinxApi) return
        try {
          const feeDataEstimate = await merlinxApi.estimateDepositFees({
            tokenContractAddress: assetReference,
            contractAddress,
      ...
      dispatch({ type: MerlinxDepositActionType.SET_DEPOSIT, payload: formValues })
      dispatch({ type: MerlinxDepositActionType.SET_LOADING, payload: true })
      try {
        // Check is approval is required for user address
        const _allowance = await merlinxApi.allowance({
          tokenContractAddress: assetReference,
          contractAddress,
          userAddress: accountAddress,
        })
        ...
        if (allowance.gte(formValues.cryptoAmount)) {
          ...
          dispatch({ type: MerlinxDepositActionType.SET_LOADING, payload: false })
        } else {
