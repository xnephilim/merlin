...
import { ASSET_NAMESPACE, ASSET_REFERENCE, toAssetId } from '@xblackfury/caip'
...
export const ClaimConfirm = ({
  ...
  const handleConfirm = useCallback(async () => {
    if (!(walletState.wallet && contractAddress && userAddress && merlinxApi && bip44Params)) return
    setLoading(true)
    try {
      if (!supportsETH(walletState.wallet))
        throw new Error(`handleConfirm: wallet does not support ethereum`)
      const txid = await merlinxApi.claimWithdraw({
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!(walletState.wallet && contractAddress && merlinxApi && chainAdapter)) return
        if (!supportsETH(walletState.wallet))
        ...
        const feeDataEstimate = await merlinxApi.estimateClaimWithdrawFees({
          claimAddress: userAddress,
          userAddress,
          contractAddress,
          wallet: walletState.wallet,
