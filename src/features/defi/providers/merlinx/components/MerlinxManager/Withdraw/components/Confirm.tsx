...
export const Confirm: React.FC<StepComponentProps & { accountId?: AccountId | undefined }> = ({
  ...
  const withdrawalFee = useMemo(() => {
    return state?.withdraw.withdrawType === WithdrawType.INSTANT
      ? bnOrZero(
          bn(state?.withdraw.cryptoAmount ?? '0').times(state?.merlinxFeePercentage ?? '0'),
        ).toString()
      : '0'
  }, [state?.withdraw.withdrawType, state?.withdraw.cryptoAmount, state?.merlinxFeePercentage])
  ...
  const handleConfirm = useCallback(async () => {
    try {
      if (
        ...
        return
      dispatch({ type: MerlinxWithdrawActionType.SET_LOADING, payload: true })

      if (!supportsETH(walletState.wallet))
        throw new Error(`handleConfirm: wallet does not support ethereum`)

      const txid = await merlinxApi.withdraw({
      ...
      dispatch({ type: MerlinxWithdrawActionType.SET_TXID, payload: txid })
      onNext(DefiStep.Status)

      const transactionReceipt = await poll({
        fn: () => merlinxApi.getTxReceipt({ txid }),
      ...
      dispatch({ type: MerlinxWithdrawActionType.SET_LOADING, payload: false })
    } catch (error) {
      console.error(error)
    }
  }, [
