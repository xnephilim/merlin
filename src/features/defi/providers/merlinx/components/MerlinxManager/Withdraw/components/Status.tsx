...
export const Status: React.FC<StatusProps> = ({ accountId }) => {
  ...
  const { stakingAsset, underlyingAsset, feeAsset, feeMarketData } = useMerlinxQuery()

  const userAddress: string | undefined = accountId && fromAccountId(accountId).account

  const withdrawalFee = useMemo(() => {
    return state?.withdraw.withdrawType === WithdrawType.INSTANT
      ? bnOrZero(bn(state.withdraw.cryptoAmount).times(state.merlinxFeePercentage)).toString()
      : '0'
    ...
    >
      <Summary spacing={0} mx={6} mb={4}>
        ...
        <Row variant='gutter'>
          ...
          <Row.Value fontWeight='bold'>{`${withdrawalFee} Merlinx`}</Row.Value>
        </Row>
        <Row variant='gutter'>
          <Row.Label>
