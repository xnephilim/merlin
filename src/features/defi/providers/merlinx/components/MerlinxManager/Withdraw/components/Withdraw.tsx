...
export type MerlinxWithdrawValues = {
...
> = ({ accountId, onAccountIdChange: handleAccountIdChange, onNext }) => {
  ...
  const methods = useForm<MerlinxWithdrawValues>({ mode: 'onChange' })
  ...
  const handleContinue = useCallback(
    async (formValues: MerlinxWithdrawValues) => {
      if (!(accountAddress && dispatch && rewardId && merlinxApi && bip44Params)) return
      ...
      const getWithdrawGasEstimateCryptoBaseUnit = async (withdraw: MerlinxWithdrawValues) => {
        if (!accountAddress) return

        try {
          const feeDataEstimate = await merlinxApi.estimateWithdrawFees({
            tokenContractAddress: rewardId,
            contractAddress,
    ...
    <FormProvider {...methods}>
      ...
      >
        <WithdrawTypeField
          asset={stakingAsset}
          handlePercentClick={handlePercentClick}
          feePercentage={bnOrZero(state.merlinxFeePercentage).toString()}
        />
      </ReusableWithdraw>
    </FormProvider>
  )
