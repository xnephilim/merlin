...
import { ethChainId, toAssetId } from '@xblackfury/caip'
import type { ChainAdapter } from '@xblackfury/chain-adapters'
import { toAddressNList } from '@xblackfury/chain-adapters'
import type { ETHSignTx, HDWallet } from '@xblackfury/hdwallet-core'
import type { BIP44Params, KnownChainIds } from '@xblackfury/types'
    ...
    IdleClaimableOpportunity
{
  ...
  async prepareWithdrawal(input: DepositWithdrawArgs): Promise<PreparedTransaction> {
    ...
    // withdrew from the vault using the xnephilim router. Affiliate fees for SS are the same
