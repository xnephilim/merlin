import type { ChainReference } from '@xblackfury/caip'
import { CHAIN_NAMESPACE, CHAIN_REFERENCE, toAssetId } from '@xblackfury/caip'
import type { EvmBaseAdapter, FeeDataEstimate } from '@xblackfury/chain-adapters'
...
import { merlinxAbi } from '../abi/merlinx-abi'
import { merlinxStakingAbi } from '../abi/merlinx-staking-abi'
...
import type {
  ...
  MerlinxAddressesType,
  MerlinxOpportunityInputData,
  GetTokeRewardAmount,
  RebaseEvent,
  RebaseHistory,
  SignAndBroadcastTx,
...
export type ConstructorArgs = {
  ...
  providerUrl: string
  merlinxAddresses: MerlinxAddressesType
  chainReference?: EthereumChainReference
}

export const transformData = ({ tvl, apy, expired, ...contractData }: MerlinxOpportunityInputData) => {
  return {
    type: DefiType.Staking,
    provider: 'ShapeShift',
    version: '1',
    contractAddress: contractData.staking,
    rewardToken: contractData.merlinx,
...
export class MerlinxApi {
  ...
  constructor({
    ...
    this.merlinxStakingContracts = merlinxAddresses.map(
      addresses => new ethers.Contract(addresses.staking, merlinxStakingAbi, this.provider),
    )
    this.liquidityReserveContracts = merlinxAddresses.map(
      addresses =>
    ...
    this.merlinxAddresses = merlinxAddresses
  }
  ...
  private getStakingContract(contractAddress: string): ethers.Contract {
    const stakingContract = this.merlinxStakingContracts.find(
  ...
  async getMerlinxOpportunities() {
    try {
      const opportunities = await Promise.all(
        this.merlinxAddresses.map(async addresses => {
          const stakingContract = this.merlinxStakingContracts.find(
            item => toLower(item.address) === toLower(addresses.staking),
          )
          try {
            const expired = await stakingContract?.pauseStaking()
            const tvl = await this.tvl({ tokenContractAddress: addresses.merlinx })
            const apy = this.apy()
      ...
      throw new Error(`getMerlinxOpportunities Error: ${e}`)
    }
  }

  async getMerlinxOpportunityByStakingAddress(stakingAddress: string) {
    this.verifyAddresses([stakingAddress])
    const addresses = this.merlinxAddresses.find(item => {
  ...
  async tvl(input: TokenAddressInput): Promise<BigNumber> {
    ...
    this.verifyAddresses([tokenContractAddress])
    const contract = new ethers.Contract(tokenContractAddress, merlinxAbi, this.provider)
  ...
  async getRebaseHistory(input: BalanceInput) {
    ...
    const merlinxContract = new ethers.Contract(tokenContractAddress, merlinxAbi, this.provider)
    const fromBlock = 14381454 // genesis rebase

    const rebaseEvents = await (async () => {
      try {
        const filter = merlinxContract.filters.LogRebase()
        const events = await merlinxContract.queryFilter(filter, fromBlock, 'latest')
    ...
    const transferEvents = await (async () => {
      try {
        const filter = merlinxContract.filters.Transfer()
        const events = await merlinxContract.queryFilter(filter, fromBlock, 'latest')
        return events
      } catch (e) {
      ...
      events.map(async event => {
        const { preRebaseBalance, postRebaseBalance } = await (async () => {
          try {
            ...
            const postRebaseBalanceResult = await merlinxContract.balanceOf(userAddress, {
              blockTag: event.blockNumber,
            })
            const unadjustedPreRebaseBalance = await merlinxContract.balanceOf(userAddress, {
    ...
    const actualResults = results.reduce<RebaseHistory[]>((acc, cur) => {
      if (cur.status === 'rejected') {
        console.error('getMerlinxRebaseHistory: balanceOf call failed - charts will be wrong')
        return acc
      }
