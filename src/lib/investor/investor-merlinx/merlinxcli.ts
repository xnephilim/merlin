...
import { MerlinxApi } from './api'
import { merlinxAddresses } from './constants'
...
const main = async (): Promise<void> => {
  ...
  const merlinxContractAddress = merlinxAddresses[0].merlinx
  const merlinContractAddress = merlinxAddresses[0].merlin
  const merlinxStakingContractAddress = merlinxAddresses[0].staking
  const liquidityReserveContractAddress = merlinxAddresses[0].liquidityReserve
  ...
  const circulatingSupply = async () => {
    try {
      const supply = await api.tvl({ tokenContractAddress: merlinxContractAddress })
  ...
  const totalSupply = async () => {
    try {
      const supply = await api.totalSupply({ tokenContractAddress: merlinxContractAddress })
  ...
  const rewardTokenBalance = async () => {
    try {
      const balance = await api.balance({
        tokenContractAddress: merlinxContractAddress,
        userAddress,
      })
    ...
    switch (index) {
      case 0:
        ...
        switch (tokenContract) {
          case 0:
            await approve(merlinContractAddress, merlinxStakingContractAddress)
            break
          case 1:
            await approve(merlinxContractAddress, merlinxStakingContractAddress)
            break
          default:
            break
        }
      ...
      case 1:
        ...
        switch (tokenContract) {
          case 0:
            ...
            break
          case 1:
            await approve(merlinxContractAddress, liquidityReserveContractAddress)
            break
          default:
            break
        }
