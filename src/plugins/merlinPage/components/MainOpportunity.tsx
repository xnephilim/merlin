...
export const MainOpportunity = ({
  ...
  const hasActiveStaking = bnOrZero(merlinxEarnOpportunityData?.stakedAmountCryptoBaseUnit).gt(0)
  ...
  const isOpportunityButtonReady = useMemo(
    () => Boolean(isDemoWallet || (wallet && !supportsETH(wallet)) || merlinxEarnOpportunityData),
    [isDemoWallet, wallet, merlinxEarnOpportunityData],
  )
