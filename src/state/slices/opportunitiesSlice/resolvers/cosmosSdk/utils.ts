...
import {
  ...
  XNEPHILIM_OSMOSIS_VALIDATOR_ADDRESS,
} from './constants'
...
export const makeUniqueValidatorAccountIds = ({
  ...
  uniq([
    toValidatorId({ account: XNEPHILIM_COSMOS_VALIDATOR_ADDRESS, chainId: cosmosChainId }),
    ...(isOsmoStakingEnabled
      ? [toValidatorId({ account: XNEPHILIM_OSMOSIS_VALIDATOR_ADDRESS, chainId: osmosisChainId })]
      : []),
...
export const getDefaultValidatorAddressFromChainId = (chainId: ChainId) => {
  switch (chainId) {
    case cosmosChainId:
      return XNEPHILIM_COSMOS_VALIDATOR_ADDRESS
    case osmosisChainId:
      return XNEPHILIM_OSMOSIS_VALIDATOR_ADDRESS
    default:
