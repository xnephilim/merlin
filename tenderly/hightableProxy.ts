...
import {
  PROXY_HIGHTABLE_ETHEREUM_HTTP_PORT,
  PROXY_HIGHTABLE_ETHEREUM_WS_PORT,
...
const { REACT_APP_HIGHTABLE_ETHEREUM_HTTP_URL, REACT_APP_HIGHTABLE_ETHEREUM_WS_URL } = dotenv.parse(
  readFileSync('.env.dev'),
)

const provider = new Web3.providers.HttpProvider(getTenderlyRpcUrl())
const web3 = new Web3(provider)
const httpPort = Number(PROXY_HIGHTABLE_ETHEREUM_HTTP_PORT)
const websocketPort = Number(PROXY_HIGHTABLE_ETHEREUM_WS_PORT)
...
const hightableWebsocket = new WebSocket(REACT_APP_HIGHTABLE_ETHEREUM_WS_URL, {
  handshakeTimeout: 5000,
})
hightableWebsocket.on('ping', val => hightableWebsocket.pong(val))
hightableWebsocket.on('error', error => console.error('[websocket:hightable]', error))
hightableWebsocket.on('connection', () =>
  console.log(
    `[websocket:hightable] connected to hightable at ${REACT_APP_HIGHTABLE_ETHEREUM_WS_URL}`,
  ),
)
...
websocketServer.on('connection', ws => {
  ...
  // client requests go to hightable
  ws.on('message', (data: RawData) => {
    captureSubscriptionDetails(data)
    hightableWebsocket.send(data)
  })

  // hightable responses
  hightableWebsocket.on('message', (data: RawData) => {
    console.error('[websocket:hightable] message:', data.toString())
    ws.send(data)
  })
})
...
app.use(
  createProxyMiddleware(['!/api/v1/account/*', '!/api/v1/send', '!/api/v1/gas/estimate'], {
    target: REACT_APP_HIGHTABLE_ETHEREUM_HTTP_URL,
    changeOrigin: true,
...
app.post('/api/v1/send', async (req, res) => {
  ...
  // emulate hightable websocket by fetching tx from tenderly and jamming it into the websocket
  setTimeout(() => {
